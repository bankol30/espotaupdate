...
short buzzercount=0;
#define SDATA RB5_BIT   //DS
#define SCLK RB3_BIT     //SH-CP
#define SCS RB2_BIT      //ST-CP
...
float  maxduty=200.0, minduty=0.0, lowduty=20.0;
...
void LoadCheck(){
if((battery>144 && battery<150) && PowerON){  //battery low BEEP
BUZZER=1;
LOWBAT=1;
}
if((battery>150 || battery<144) && PowerON){  //battery low BEEP
BUZZER=0;
LOWBAT=0;
}
if(battery<=144 && PowerON){  //battery low
FaultOccur=1;
inv_mask=0;
RELAY=0;
BUZZER=0;
FaultTag=1;
 ...
 if(temperature<=60 && PowerON ){  //over temperature
 FaultOccur=1;
 BUZZER=1;
 RELAY=0;
 FAN=1;
...
//ups mode
if(ra4_bit==1){
if ( output > 120) DutyCycle -= 0.01;
else if ( output < 115) DutyCycle += 0.01;
if (DutyCycle <= 0.1) DutyCycle = 0.1;
else if (DutyCycle >= 1.0) DutyCycle = 1.0;
}
////inverter mode
 else if(ra4_bit==0){
 if ( output > 120) DutyCycle -= 0.0002;
else if ( output < 115) DutyCycle += 0.0002;
if (DutyCycle <= 0.1) DutyCycle = 0.1;
else if (DutyCycle >= 1.0) DutyCycle = 1.0;
 }
}
//////////////////////////////////////////charging phase  when inverter is on
else if(mainsdetect && (inv_mask || !inv_mask) && (temperature>70 && temperature<128)){     //when grid available  and inverter is on
delay_us(2);
RELAY=0;
FAN=1;
T1CON=0;
...
if(battery >= 115 && battery <= 193){
minduty+=0.001;
if(minduty>=maxduty)
minduty=maxduty;
}
else if(battery>193 && battery<=194){
minduty-=0.001;
if(minduty==lowduty)
minduty=lowduty;
charging_timer=0;
