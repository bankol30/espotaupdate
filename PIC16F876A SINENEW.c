...
float  maxduty=150.0, minduty=40.0, lowduty=20.0;
...
void LoadCheck(){
if(battery<600 && PowerON){  //battery low
FaultOccur=1;
inv_mask=0;
RELAY=0;
BUZZER=0;
FaultTag=1;
 ...
 if(temperature<=300 && PowerON){  //over temperature
 FaultOccur=1;
 BUZZER=1;
 RELAY=0;
 inv_mask=0;
 ...
 if(temperature<=300 && mainsdetect){  //over temperature
 FaultOccur=1;
 BUZZER=1;
 minduty=lowduty;
 mainsdetect=0;
...
//ups mode
if(ra4_bit==1){
if ( output > 450) DutyCycle -= 0.01;
else if ( output < 430) DutyCycle += 0.01;
if (DutyCycle <= 0.1) DutyCycle = 0.1;
else if (DutyCycle >= 1.0) DutyCycle = 1.0;
}
////inverter mode
 else if(ra4_bit==0){
   if ( output > 450) DutyCycle -= 0.0002;
else if ( output < 430) DutyCycle += 0.0002;
if (DutyCycle <= 0.1) DutyCycle = 0.1;
else if (DutyCycle >= 1.0) DutyCycle = 1.0;
 }
}
...
if(battery > 512 && battery < 700){
minduty+=0.01;
if(minduty>=maxduty)
minduty=maxduty;
}
else if(battery>700 && battery<=750){
minduty-=0.01;
if(minduty==lowduty)
minduty=lowduty;
}
