...
char txt3[] = "INVERTER ON";
char txt4[] = "INVERTER OFF";
...
#define VACANTPIN RC7_BIT //VACANT PIN
*/
...
int  FaultTag;
unsigned int  GridTimer=0, GridCount=0;
short PowerON=0, FaultOccur=0,  chargestart=0, mainscheck=0, mainsdetect=0, chargePhase=0;
int ii;
short ERROR_FLAG=0;
...
if((battery>512 && battery<620) && PowerON){  //battery low BEEP
BUZZER=1;

}
if((battery>620 || battery<512) && PowerON){  //battery low BEEP
BUZZER=0;

}
if(battery<=512 && PowerON){  //battery low
FaultOccur=1;
inv_mask=0;
RELAY=0;
BUZZER=0;
...
PORTC=0x00;
 ...
 Lcd_Chr(1,2,'P'); Lcd_Chr(1,3,'u'); Lcd_Chr(1,4,'r'); Lcd_Chr(1,5,'e'); Lcd_Chr(1,6,'s'); Lcd_Chr(1,7,'i');    Lcd_Chr(1,8,'n'); Lcd_Chr(1,9,'e');
...
for(;;){
LoadCheck();
if((battery>512 && battery<800) && PowerON){
 Lcd_Out(1,3,"INVERTER ON");
}
}
}