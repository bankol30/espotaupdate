const int look_up[64]=  { 128, 141, 154, 166, 179, 191, 203, 214, 225, 235, 244, 253, 255, 253, 244, 235, 225, 214, 203, 191, 179, 166, 154, 141,
  128, 115, 102, 90, 77, 65, 53, 42, 31, 21, 12, 3, 0, 3, 12, 21, 31, 42, 53, 65, 77, 90, 102, 115
};
unsigned int duty=0;
...
INTE_BIT=0;
 ...
 short hysteresis=0;
void LoadCheck(){
if((battery>440 || battery<392) && PowerON){  //battery low BEEP
BUZZER=0;
LOWBAT=0;
}
if(battery<=392 && PowerON){  //battery low
recovery_counter++;
...
FaultTag=1;
 ...
 if(temperature<=370 && PowerON ){  //fan start
 FAN=1;
 }
  if(temperature>420 && PowerON ){  //fan stop
 FAN=0;
 }
 if(temperature<=120 && PowerON ){  //over temperature
 FaultOccur=1;
 BUZZER=1;
 LOWBAT=0;
 RELAY=0;
...
if ( output > 512) DutyCycle -= 0.05;
else if (output < 480) DutyCycle += 0.05;
